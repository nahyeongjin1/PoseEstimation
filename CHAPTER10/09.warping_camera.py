...
def findCorners(image, bSize):
    ...
    if ret:
        criteria = (cv2.TermCriteria_MAX_ITER + cv2.TERM_CRITERIA_EPS, 30, 0.1)
...
def calibrate_correct(objectPoints, imagePoints, image):
    ...
    newScale, roi = cv2.getOptimalNewCameraMatrix(ret[1], ret[2], size, 1)
    undistorted = cv2.undistort(image, ret[1], ret[2], None, newScale)
    x, y, w, h = roi
...
images = [cv2.imread("images/%s.jpg" % file, cv2.IMREAD_COLOR) for file in files]
